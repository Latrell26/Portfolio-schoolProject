<script>
    ...
    let forceHideAfterClick = false; // Bagong flag: puwersahang mag-hide pagkatapos ng click
    ...
    onMount(() => {
        const handleScroll = () => {
            ...
            // Kung may pending na force hide (galing sa click), hide agad
            if (forceHideAfterClick) {
                navbarVisible = false;
                forceHideAfterClick = false; // reset after apply
            } 
            // Normal manual scroll behavior
            else {
            ...
            // Always show kapag nasa pinakatop
    ...
    <div class="flex justify-between items-center max-w-300 mx-auto px-4">
        ...
        <ul class="hidden md:flex gap-12 text-lg">
            <li class="hover:underline underline-offset-4 cursor-pointer">
                <a 
                    href="#home" 
                    on:click={() => forceHideAfterClick = true}
...
>
    ...
    <ul class="flex flex-col gap-6 text-xl">
        <li>
            <a 
                href="#home" 
                on:click={() => { menuOpen = false; forceHideAfterClick = true; }}
                class={activeSection === "home" ? "underline" : ""}
            >Home</a>
        </li>
        <li>
