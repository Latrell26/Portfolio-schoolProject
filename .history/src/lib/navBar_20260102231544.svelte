...
<nav
  class={`fixed top-0 left-0 right-0 z-50 py-2 transition-all duration-300 ease-out
...
>
  <div class="flex justify-between items-center max-w-7xl mx-auto px-4">
   ...
   <button class="md:hidden text-3xl font-bold z-90" on:click={() => menuOpen = !menuOpen}>
  {#if menuOpen}
    ✕
  {:else}
    ☰
...
<div
  class={`fixed top-0 right-0 h-full w-64 bg-black/80  backdrop-blur-xl text-white p-6 z-80
    transition-transform duration-300 ${menuOpen ? "translate-x-0" : "translate-x-full"}`}
>

  <button
  on:click={toggleDarkMode}
  class="relative w-14 h-7 rounded-full bg-white shadow-md flex items-center cursor-pointer mb-6 z-80"
>
  <div
    class="w-7 h-7 rounded-full transition-all duration-300 flex items-center justify-center text-white
           {isDark ? 'bg-dark translate-x-7' : 'bg-yellow-500 translate-x-0'}"
  >
    {@html currentIcon}
  </div>
