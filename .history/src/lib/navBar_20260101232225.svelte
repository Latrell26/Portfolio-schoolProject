<script>
    ...
    // Para sa hide/show navbar on scroll
    let prevScrollY = 0;
    let navbarVisible = true;
    let isProgrammaticScroll = false; // Flag para sa automatic scroll from links
    ...
    onMount(() => {
        const handleScroll = () => {
            ...
            // Hide/show logic — ignore kung programmatic (from link click)
            ...
            // Reset flag after smooth scroll (safe delay)
            if (isProgrammaticScroll) {
                setTimeout(() => {
    ...
    <div class="flex justify-between items-center max-w-300 mx-auto px-4">
        ...
        <ul class="hidden md:flex gap-12 text-lg">
            <li class="hover:underline underline-offset-4 cursor-pointer">
                <a 
                    href="#home" 
                    on:click={() => isProgrammaticScroll = true}
...
>
    ...
    <ul class="flex flex-col gap-6 text-xl">
        <li>
            <a 
                href="#home" 
                on:click={() => { menuOpen = false; isProgrammaticScroll = true; }}
                class={activeSection === "home" ? "underline" : ""}
            >
                Home
            </a>
...
<!-- Optional: Para smooth yung scroll sa links -->
<svelte:head>
    <style>
        html {
