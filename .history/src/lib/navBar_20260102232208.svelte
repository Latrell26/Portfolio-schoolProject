<script>
  ...
  onMount(() => {
    ...
    const handleScroll = () => {
      ...
      if (currentScrollY > prevScrollY && currentScrollY > 100) navbarVisible = false;
      else if (currentScrollY < prevScrollY) navbarVisible = true;
...
>
  <div class="flex justify-between items-center max-w-7xl mx-auto px-4">
    ...
    <!-- MOBILE LEFT: Dark Toggle -->
    <button
      on:click={toggleDarkMode}
      class="md:hidden w-14 h-7 rounded-full bg-white shadow-md flex items-center justify-center z-50 mr-2"
    >
...
>
  <ul class="flex flex-col gap-6 text-xl mt-12">
    {#each ["home","about","education","projects","contact"] as item}
      <li>
